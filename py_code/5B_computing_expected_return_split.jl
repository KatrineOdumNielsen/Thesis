# ====================================================================================
#
#         Part 5B: Estimating the expected returns and model-predicted alphas
#
...
mu_hat = zeros(4,1)
theta_i_hat = zeros(4,1)
exp_exc_ret = zeros(4,1)
capm_alpha = zeros(4,1)
...
bound = [20,20,20,2.5]       # list for bounds of integrals
...
mu1, mu2, mu3, mu4              = mu_hat[1], mu_hat[2], mu_hat[3] , mu_hat[4]
theta1, theta2, theta3, theta4  = theta_i_hat[1], theta_i_hat[2], theta_i_hat[3], theta_i_hat[4]
exp1, exp2, exp3, exp4          = exp_exc_ret[1], exp_exc_ret[2], exp_exc_ret[3], exp_exc_ret[4]
mkt_ret                         = market_return
alpha1, alpha2, alpha3, alpha4  = capm_alpha[1], capm_alpha[2], capm_alpha[3], capm_alpha[4]
bound_di_low, bound_di_high, bound_hy, bound_ig = bound[1], bound[2], bound[3], bound[4]
...
log_row = DataFrame(
  ...
  Ri             = Ri,
  mu             = mu,
  bound_di_low   = bound_di_low,
  bound_di_high  = bound_di_high,
  bound_hy       = bound_hy,
  bound_ig       = bound_ig,
  mu1            = mu1,
  mu2            = mu2,
